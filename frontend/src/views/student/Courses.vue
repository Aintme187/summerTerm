...
function deleteCourse(id) {
  deleteCourseApi(id).then(() => {
    ElMessage({type: "success", message: "成功删除教学班"})
    getCourses()
  }).catch((reason) => {
...
function deleteSelectedCourses() {
  batchDeleteCoursesApi(getSelectedIds()).then(() => {
    ElMessage({type: "success", message: "成功批量删除教学班"})
    getCourses()
  }).catch((reason) => {
    ElMessage({type: "error", message: reason})
  })
...
<template>
  ...
  <div style="flex: 1; min-width: 0; display: flex; flex-direction: column">
    ...
    <!--key="Courses" 显示复选框-->
    <el-table v-loading="isLoading" ref="CoursesRef" :data="courseData" border key="Courses" style="margin: 10px 0; flex: 1">
