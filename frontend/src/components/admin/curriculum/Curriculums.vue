...
import {batchDeleteCurriculumsApi, deleteCurriculumApi, getCurriculumsApi} from "@/api/adminCurriculum.js";
...
const columns = {
  ...
  "name": {"type": "string", "label": "课程组名称"},
  "credit": {"type": "double", "label": "学分"},
  "introduction": {"type": "string", "label": "课程组简介"},
}
const enum2label = {}
/**
 * 筛选条件
...
function getCurriculums() {
  loading.value = true
  getCurriculumsApi(page.value, pageSize.value, filterData.value).then((value) => {
    total.value = value.data["curriculumCount"]
    curriculumData.value = value.data["curriculumList"]
  })
  loading.value = false
}

function deleteCurriculum(id) {
  deleteCurriculumApi(id).then(() => {
...
function editCurriculum(id) {
  router.push("/admin/curriculums/info/" + id)
}

function addCurriculum() {
  router.push("/admin/curriculums/add")
}
...
function getSelectedIds() {
  if (!CurriculumsRef || !CurriculumsRef.value || !CurriculumsRef.value.getSelectionRows()) return []
  let ids = []
  // rows=[{prop:value,...},...]
  let rows = CurriculumsRef.value.getSelectionRows()
  for (let row in rows) {
    // for in list 得到的是索引
...
function editSelectedCurriculums() {
  router.push("/admin/Curriculums/edit/" + getSelectedIds())
}

function deleteSelectedCurriculums() {
  batchDeleteCurriculumsApi(getSelectedIds()).then(() => {
...
<template>
  ...
  <div style="flex: 1; min-width: 0; display: flex; flex-direction: column">
    <div>
      <el-button type="success" style="padding: 10px" @click="addCurriculum()">
        新增
      ...
      <el-button type="warning" style="padding: 10px" @click="editSelectedCurriculums()"
      ...
      <el-button type="danger" style="padding: 10px" @click="deleteSelectedCurriculums()"
      ...
      <el-button type="info" style="padding: 10px" @click="getCurriculums()">
        刷新
    ...
    <!--key="Curriculums" 显示复选框-->
    <el-table ref="CurriculumsRef" :data="curriculumData" border key="Curriculums" :v-loading="loading"
      ...
      <el-table-column fixed="right" label="操作">
        <template #default="scope">
          <el-link type="warning" @click="editCurriculum(scope.row.id)" style="margin-right: 10px">
          ...
          <el-link type="danger" @click="deleteCurriculum(scope.row.id)">
