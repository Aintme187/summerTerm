...
import {batchDeleteCoursesApi, deleteCourseApi, getCoursesApi} from "@/api/adminCourse.js";
...
const columns = {
  ...
  "curriculumId": {"type": "integer", "label": "课程组id"},
  "curriculumName": {"type": "string", "label": "课程组名称"},
  "credit": {"type": "double", "label": "学分"},
  "introduction": {"type": "string", "label": "课程组简介"},
  "enrollment": {"type": "integer", "label": "选课人数"},
  "capacity": {"type": "integer", "label": "课程组容量"},
  "teacherId": {"type": "integer", "label": "教师id"},
...
function getCourses() {
  loading.value = true
  getCoursesApi(page.value, pageSize.value, filterData.value).then((value) => {
    total.value = value.data["courseVoCount"]
    courseData.value = value.data["courseVoList"]
  })
  loading.value = true
}
...
function deleteCourse(id) {
  deleteCourseApi(id).then(() => {
    ElMessage({type: "success", message: "成功删除课程"})
    getCourses()
  }).catch((reason) => {
...
function editCourse(id) {
  router.push("/admin/courses/info/" + id)
}
...
function getSelectedIds() {
  if (!CoursesRef || !CoursesRef.value || !CoursesRef.value.getSelectionRows()) return []
  let ids = []
  // rows=[{prop:value,...},...]
  let rows = CoursesRef.value.getSelectionRows()
  for (let row in rows) {
    // for in list 得到的是索引
...
function editSelectedCourses() {
  router.push("/admin/Courses/edit/" + getSelectedIds())
}

function deleteSelectedCourses() {
  batchDeleteCoursesApi(getSelectedIds()).then(() => {
    ElMessage({type: "success", message: "成功批量删除课程"})
    getCourses()
  }).catch((reason) => {
    ElMessage({type: "error", message: reason})
  })
...
<template>
  ...
  <div style="flex: 1; min-width: 0; display: flex; flex-direction: column">
    <div>
      <el-button type="success" style="padding: 10px" @click="addCourse()">
        新增
      ...
      <el-button type="warning" style="padding: 10px" @click="editSelectedCourses()"
      ...
      <el-button type="danger" style="padding: 10px" @click="deleteSelectedCourses()"
      ...
      <el-button type="info" style="padding: 10px" @click="getCourses()">
        刷新
    ...
    <!--key="Courses" 显示复选框-->
    <el-table ref="CoursesRef" :data="courseData" border key="Courses" :v-loading="loading"
      ...
      <el-table-column fixed="right" label="操作">
        <template #default="scope">
          <el-link type="warning" @click="editCourse(scope.row.id)" style="margin-right: 10px">
          ...
          <el-link type="danger" @click="deleteCourse(scope.row.id)">
